...
export default {
  setup() {
    ...
    const { state, increment } = counterContainer.inject()
...
### createContainer(useComposed: function): Container
```js
import { reactive } from '@vue/composition-api'
import { createContainer } from 'vue-unstated'
...
### Container.inject(): Composition
```js
export default {
  setup() {
    // 'inject' function must be called in 'setup' function.
    const { state, increment, decrement } = counterContainer.inject()
  }
}
```
